...
class MultiSignatureTransaction
  def initialize buyer_publick_key, seller_public_key, escrow_public_key
    @buyer_publick_key = buyer_publick_key
    @buyer_address = Bitcoin.pubkey_to_address @buyer_pubilc_key

    @seller_public_key = seller_public_key
    @seller_address = Bitcoin.pubkey_to_address @seller_pubilc_key

    @escrow_public_key = escrow_public_key

    @client = BitcoinRPC.new('http://username:password@127.0.0.1:18332')

    multi_sig = @client.createmultisig(2, [buyer_publick_key, seller_public_key, escrow_public_key])
  ...
  def create_and_send prev_tx_id, buyer_private_key, starting_amount, send_amount
    tx = @client.createrawtransaction [{txid: prev_tx_id, vout: 1}], {@multi_sig_address => send_amount, @buyer_address => starting_amount - send_amount}
  ...
  def vout_to_multi_sig_address
    @funding_tx['vout'].detect{|o| o['scriptPubKey']['addresses'].include?(@multi_sig_address)}
  end
end
